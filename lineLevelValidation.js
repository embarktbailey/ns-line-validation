...
define(["N/record", "N/log"], function (record, log) {
  ...
  function fetchCustomRecordData() {
    try {
      ...
      CRAccountType = customRecord.getText({
        fieldId: "custrecord_account_type",
      });
      DepartmentOverride = customRecord.getValue({
        fieldId: "custrecord_department_override",
      });
      ClassOverride = customRecord.getValue({
        fieldId: "custrecord_class_override",
      });
      LocationOverride = customRecord.getValue({
        fieldId: "custrecord_location_override",
      });
      ExcludeAccounts =
        customRecord.getValue({ fieldId: "custrecord_exclude_accounts" }) || [];
      IncludeAccounts =
        customRecord.getValue({ fieldId: "custrecord_include_accounts" }) || [];
      ...
      DepartmentMandatory = customRecord.getValue({
        fieldId: "custrecord_department_mandatory",
      });
      LocationMandatory = customRecord.getValue({
        fieldId: "custrecord_location_mandatory",
      });
      ClassMandatory = customRecord.getValue({
        fieldId: "custrecord_class_mandatory",
      });
      NameMandatory = customRecord.getValue({
        fieldId: "custrecord_name_entity_mandatory",
      });
  ...
  function validateFields(context) {
    ...
    for (var i = 0; i < lineCount; i++) {
      ...
      if (
        accountType != CRAccountType &&
        IncludeAccounts.indexOf(account) == -1
      ) {
        log.debug(
          "Is Account Included on line " + i + ":",
          IncludeAccounts.indexOf(account) == -1
        );
      ...
      if (
        accountType == CRAccountType &&
        ExcludeAccounts.indexOf(account) != -1
      ) {
        log.debug(
          "Is Account Included on line " + i + ":",
          ExcludeAccounts.indexOf(account) == -1
